    ...
    return (
        <div>
            ...
            <h2>
                {chats.length === 0 
                && 
                <div>У вас пока нет переписок</div>}
            </h2>
            ...
            <div>
                {
                    chats.map((chat) => {
                        ...
                        onClick={() => navigate(`/chat/${chat.product_id}`,
                           {state:{sender:companionId}} 

                        )}  key={chat.id} >
                            <img 
                                src={chat.product_image} 
                                alt={chat.product_name} width={100}
                            />
                            <div>
                               ...
                               {chat.unread_count > 0 && (
                                <span  style={{color:'red'}} >{chat.unread_count} непрочитанных</span>
                               )}
                            </div>
